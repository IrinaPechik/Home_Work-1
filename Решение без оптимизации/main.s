...
main:
	...
	mov	rbp, rsp									# | rbp := rsp
	sub	rsp, 224									# \ rsp -= 224
	mov	DWORD PTR -212[rbp], edi 					# edi - первый аргумент - args, записали его на стек, отступая от rbp на -212 (стек растёт вниз)
...
.L17:
	...
	lea	rcx, .LC5[rip]							# запись в rax output на -32 позиции
	mov	rsi, rcx
	mov	rdi, rax
	mov	eax, 0
	call	fprintf@PLT
