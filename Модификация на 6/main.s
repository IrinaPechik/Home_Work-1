	# -196[rbp] -> r15d size_a
	# -4[rbp] -> r12d - i
	# -8[rbp] -> r13d - локальная i
	# -12[rbp] -> r14d - локальная-локальная i
	# -16[rbp] -> r11d - size_b
	# -212[rbp] - argc
	# -224[rbp] - argv
	# -192[rbp] - B
...
main:
	...
	mov eax, r15d 									# запись size_a регистр r15d в eax
...
.L4:
	...
	jmp	.L6
.L7:
	mov	eax, r12d						# записываем i в eax в регистр r12d
	mov	esi, eax
	lea	rax, .LC3[rip]
	mov	rdi, rax
	mov	eax, 0
	...
	add	r12d, 1					   # смещаем на 1 позицию i
.L6:
	mov eax, r15d 								# запись size_a регистр r15d в eax
	mov	eax, DWORD PTR -196[rbp]				# запись в eax size_a на позиции -196
	cmp	r12d, eax					# сравнение size_a и i
	jl	.L7
	mov ecx, r15d 								# запись size_a регистр r15d в eсx
	...
	mov	rdi, rax
	call	Task@PLT
	mov	r11d, eax						# запись eax в size_b на позиции r11d
	mov	DWORD PTR -16[rbp], eax					# запись в size_b из eax на позиции -16
	lea	rax, .LC4[rip]
	mov	rdi, rax
	mov	eax, 0
	call	printf@PLT
	mov	r13d, 0					# присваивание 0 локальной i в регистр r13d
	jmp	.L8
.L9:
	mov	eax, r13d				# записываем в eax локальной i в регистр r13d
	cdqe
	...
	mov	eax, 0
	call	printf@PLT
	add	r13d, 1					# присваивание 1 локальной i в регистр r13d
.L8:
	mov	eax, r13d					# записываем в eax локальную i в регистр r13d
...
.L2:
	...
	mov	rdi, rax
	call	atoi@PLT
	mov r15d, eax 											# запись eax в size_a регистр r15d 
...
.L13:
	...
	jmp	.L14
.L15:
	mov	eax, r12d					# запись в eax в регистр r12d локальной i 
	cdqe
	...
	add	r12d, 1							# запись в i в регистр r12d единицы
.L14:
	mov eax, r15d 								# запись size_a регистр r15d в eax
	mov	eax, DWORD PTR -196[rbp]						 # запись size_a в eax на позиции -292
	cmp	r12d, eax							 # запись в i в регистр r12d eax 
	jl	.L15
	mov ecx, r15d 									#запись size_a регистр r15d в ecx
	...
	mov	r12d, 0								# запись в i в регистр r12d нуля
	jmp	.L16
.L17:
...
.L11:
	...
	mov	rdi, rax
	call	atoi@PLT
	mov r15d, eax 									#запись eax в size_a в регистр r15d
	mov	DWORD PTR -196[rbp], eax								# запись в size_a eax, расположенной на позиции -196
	mov	edi, 0
	call	time@PLT
	mov	edi, eax
	call	srand@PLT
	mov r15d, 0 									#запись 0 в size_a в регистр r15d
...
.L18:
	...
	cmp	eax, 20
	jg	.L19
	mov	r12d, 0						# присваивание 0 i в регистр r12d
	jmp	.L20
.L22:
	call	rand@PLT
	mov	edx, r12d			# запись в edx в регистр r12d локальной i
	movsx	rdx, edx
	...
	test	eax, eax
	jne	.L21
	mov	eax, r12d				# запись в eax локальной i в регистр r12d
	cdqe
...
.L20:
	...
	mov	eax, 0
	call	printf@PLT
	mov	r12d, 0			# присваивание нуля i на позиции -4 
	jmp	.L23
.L24:
	mov	eax, r12d         # запись i в eax на позиции -4
	cdqe
	...
	mov	eax, 0
	call	printf@PLT
	add	r12d, 1			   # присваивание единички i в регистр r12d
.L23:
	mov eax, r15d 							#запись size_a регистр r15d в eax
	mov	eax, DWORD PTR -196[rbp]		   # запись в eax size_a на позиции -196
	cmp	r12d, eax			   # сравнивнение size_a и i
	jl	.L24
	mov	edi, 10
	call	putchar@PLT
	mov ecx, r15d 							#запись size_a регистр r15d в ecx
	...
	mov	eax, 0
	call	printf@PLT
	mov	r14d, 0				# запись 0 в локальную-локальную i в регистр r14d
	jmp	.L25
.L26:
	mov	eax, r14d				# запись локальной-локальной i в регистр r14d
	cdqe
	...
	mov	eax, 0
	call	printf@PLT
	add	r14d, 1				# запись 1 в  локальную-локальную i в регистр r14d
.L25:
	mov	eax, r14d             # запись локальной-локальной i в регистр r14d
